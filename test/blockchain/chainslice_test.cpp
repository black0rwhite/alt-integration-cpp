...
/*
TEST_F(ChainSliceTestFixture, DefaultSlice) {
  slice_t defaultSlice;

  ASSERT_EQ(defaultSlice.size(), 0);
  ASSERT_TRUE(defaultSlice.empty());
  ASSERT_EQ(defaultSlice.first(), nullptr);
  ASSERT_EQ(defaultSlice.tip(), nullptr);
}
*/

TEST_F(ChainSliceTestFixture, EmptySlice) {
  slice_t emptySlice{chain, chain.firstHeight() + 1, 0};

  ASSERT_EQ(emptySlice.size(), 0);
  ASSERT_TRUE(emptySlice.empty());
  ASSERT_EQ(emptySlice.first(), nullptr);
  ASSERT_EQ(emptySlice.tip(), nullptr);
}

TEST_F(ChainSliceTestFixture, CopySliceSize) {
  slice_t copySlice(chain);

  ASSERT_EQ(chain.firstHeight(), copySlice.firstHeight());
  ASSERT_EQ(chain.size(), copySlice.size());
  ASSERT_EQ(chain.first(), copySlice.first());
  ASSERT_EQ(chain.tip(), copySlice.tip());
}

TEST_F(ChainSliceTestFixture, OffsetSliceSize) {
  auto const offset = 2;
  slice_t offsetSlice(chain, chain.firstHeight() + offset);

  ASSERT_EQ(chain.firstHeight() + offset, offsetSlice.firstHeight());
  ASSERT_EQ(chain.size(), offsetSlice.size() + offset);
  ASSERT_EQ(chain[chain.firstHeight() + offset], offsetSlice.first());
  ASSERT_NE(offsetSlice.first(), nullptr);
  ASSERT_EQ(chain.tip(), offsetSlice.tip());
}

TEST_F(ChainSliceTestFixture, SubseSliceSize) {
  auto const offset = 5;
  auto const size = 7;
  slice_t subsetSlice(chain, chain.firstHeight() + offset, size);

  ASSERT_EQ(chain.firstHeight() + offset, subsetSlice.firstHeight());

  ASSERT_NE(subsetSlice.first(), nullptr);
  ASSERT_NE(subsetSlice.tip(), nullptr);

  ASSERT_EQ(chain[chain.firstHeight() + offset], subsetSlice.first());
  ASSERT_EQ(chain[chain.firstHeight() + offset + size - 1], subsetSlice.tip());
}
